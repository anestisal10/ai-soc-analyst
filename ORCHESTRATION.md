...
- **Caching First**: The pipeline in `main.py` utilizes `diskcache` via `services/cache.py`. Always check the cache using the SHA-256 hash of the input BEFORE hitting external APIs or LLMs.
- **LLM Specialization (Dual-Brain)**: 
  - **Cerebras (`gpt-oss-120b`)** handles pure *Technical Analysis* (IoC extraction, payload details, HTTP header anomalies).
  - **Groq / Kimi K2** handles *Psychological/Contextual Analysis* (Social engineering tactics, urgency, tone). Model: `moonshotai/kimi-k2-instruct-0905` via Groq.
  - *Rule*: Do NOT mix these responsibilities. If you need new behavioral insights, modify the Groq/Kimi K2 prompt. If you need new technical extractions, modify the Cerebras prompt.
- **OSINT Enrichment**:
