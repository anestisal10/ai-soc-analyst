...
- **Caching Engine** (`cache.py`): Hashes input content to detect returning threats and instantly returns cached reports, bypassing all heavy computation.
...
- **Exporter** (`pdf_exporter.py`): Converts `ThreatReport` into a downloadable PDF format.
...
- **Technical Engine (Cerebras / `gpt-oss-120b`)**: 
  ...
  - Served via `cerebras-cloud-sdk`; synchronous SDK run in `asyncio.to_thread` for non-blocking execution.
- **Psychological Engine (Groq / Kimi K2 Instruct)**: 
  - Profiles social engineering tactics (Fear, Greed, Authority Imitation).
  - Analyzes the emotional payload and urgency mechanics.
  - Model: `moonshotai/kimi-k2-instruct-0905` served via Groq inference.
